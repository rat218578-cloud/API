...
export function GameLauncher({ isOpen, onClose }: GameLauncherProps) {
  const { getAllGameLinks, isAuthenticated } = useAuth();
  const [links, setLinks] = useState<Record<string, string | null>>({});
  const [loading, setLoading] = useState(false);
  const [selectedGame, setSelectedGame] = useState<string | null>(null);

  useEffect(() => {
    if (isOpen && isAuthenticated && Object.keys(links).length === 0) loadGameLinks();
      ...
      <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-bg-card border border-border-default rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden slide-in">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {Object.entries(JOGOS).map(([key, jogo]) => {
                  const url = links[key];
                  return (
                    <div key={key} onClick={() => url && setSelectedGame(key)}
                      className={`p-4 rounded-xl border transition-all ${
                        url ? 'bg-bg-tertiary border-border-default hover:border-accent-pink cursor-pointer hover:scale-105' :
          ...
          <div className="p-4 border-t border-border-default flex justify-between items-center">
            ...
            <button onClick={loadGameLinks} disabled={loading}
        ...
        <div className="fixed inset-0 z-[110] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-bg-card border border-border-default rounded-2xl w-full max-w-6xl h-[90vh] overflow-hidden slide-in">
            <div className="flex items-center justify-between p-3 border-b border-border-default">
              ...
              <div className="flex items-center gap-2">
                <a href={links[selectedGame]!} target="_blank" rel="noopener noreferrer"
                ...
                <button onClick={() => setSelectedGame(null)} className="p-2 rounded-lg hover:bg-bg-tertiary text-text-secondary">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            <div className="w-full h-[calc(100%-60px)]">
              <iframe src={links[selectedGame]!} className="w-full h-full border-0"
                allow="autoplay; fullscreen; camera; microphone" loading="lazy"
