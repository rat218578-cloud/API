      ...
      <div className="flex items-center gap-4">
        <div>
          <h1 className="text-lg font-bold text-text-primary flex items-center gap-2">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full bg-bg-tertiary border border-border-default text-text-secondary">{title}</span>
          </h1>
          {subtitle && <p className="text-xs text-text-muted">{subtitle}</p>}
        </div>
      </div>
      <div className="flex items-center gap-3">
        <a href="https://sortenabet.com" target="_blank" rel="noopener noreferrer"
        ...
        <button className="hidden md:flex items-center gap-2 px-3 py-2 rounded-xl bg-bg-tertiary border border-border-default text-text-secondary hover:border-border-hover transition-colors text-sm">
          ...
          <span className="text-text-primary font-semibold">{balance.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
        </button>
        ...
        <button className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-xl bg-bg-tertiary border border-border-default hover:border-border-hover transition-colors group">
          ...
          <div className="hidden sm:block text-left">
            ...
            <div className="text-[10px] text-text-muted flex items-center gap-1">
              <Crown className="w-3 h-3 text-amber-400" /> {user?.plan || 'Pro'}
            </div>
          </div>
          {onLogout && (
            <button onClick={onLogout} className="ml-2 p-1 rounded-lg hover:bg-bg-tertiary text-text-muted hover:text-red-400 transition-colors">
