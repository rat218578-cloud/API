...
export function RouletteDashboard() {
  ...
  useEffect(() => { fetchRouletteData(activeRoom); }, [activeRoom, fetchRouletteData]);

  const openGame = (slug: string) => { setSelectedSlug(slug); setShowVideo(true); };
  const closeGame = () => { setShowVideo(false); setSelectedSlug(null); };
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 flex-wrap">
        {ROLETAS.map((r) => (
          <button key={r.id} onClick={() => { setActiveRoom(r.id); openGame(r.slug); }}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-colors border ${activeRoom === r.id ? "bg-bg-tertiary border-border-hover text-text-primary" : "bg-bg-card border-border-default text-text-secondary hover:border-border-hover"}`}>
            {r.nome} <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          </button>
        ))}
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
        <div className="xl:col-span-3 bg-bg-card border border-border-default rounded-2xl p-4">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-bold text-text-primary text-sm">📊 CATALOGAÇÃO</h3><p className="text-[10px] text-text-muted uppercase">Dados ao vivo</p></div>
            <button onClick={refreshHistory} disabled={loading} className="px-2 py-1 rounded-lg bg-emerald-500/20 text-emerald-400 text-xs font-medium flex items-center gap-1">
              {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />} AO VIVO
            </button>
          </div>
        ...
        <div className="xl:col-span-6">
          {showVideo && selectedSlug ? (
            ...
            <div className="bg-bg-card border border-border-default rounded-2xl p-8 flex flex-col items-center justify-center min-h-[400px]">
              ...
              <p className="text-text-muted text-sm text-center max-w-md">Selecione uma roleta no topo para ver o jogo ao vivo</p>
            </div>
          )}
        </div>
