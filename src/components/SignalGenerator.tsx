...
export function SignalGenerator({ history }: SignalGeneratorProps) {
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < 3; i++) newSignals.push(generateAISignal(history));
    ...
    <div className="bg-bg-card border border-border-default rounded-2xl p-4">
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setActiveTab("all")}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
            activeTab === "all" ? "bg-bg-tertiary text-text-primary border border-border-hover" : "text-text-secondary hover:text-text-primary"
          }`}>Todos os sinais</button>
        <button onClick={() => setActiveTab("favorites")}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
            activeTab === "favorites" ? "bg-bg-tertiary text-text-primary border border-border-hover" : "text-text-secondary hover:text-text-primary"
          }`}>Favoritos</button>
      </div>
      <div className="space-y-2 max-h-[320px] overflow-y-auto">
        {signals.length === 0 ? (
          ...
          signals.map((signal) => (
            <div key={signal.id} className="p-3 rounded-xl bg-bg-tertiary/50 border border-border-default hover:border-border-hover transition-colors slide-in">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-accent-pink">{signal.strategy}</span>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                    signal.assertiveness >= 85 ? "bg-emerald-500/20 text-emerald-400" :
                    signal.assertiveness >= 70 ? "bg-amber-500/20 text-amber-400" : "bg-red-500/20 text-red-400"
                  }`}>{signal.assertiveness}% assertividade</span>
                </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                {signal.numbers.map((n) => (
                  <div key={n} className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${
                    n === 0 ? "number-green" :
                    [1,3,5,7,9,12,14,16,18,19,21,23,25,27,30,32,34,36].includes(n) ? "number-red" : "number-black"
                  }`}>{n}</div>
                ))}
              </div>
              <p className="text-[11px] text-text-secondary flex items-start gap-1">
                <Target className="w-3 h-3 mt-0.5 text-accent-cyan" /> {signal.reason}
              </p>
            </div>
          ))
        )}
