...
interface LiveGameViewProps { slug: string; isOpen: boolean; onClose: () => void; }
...
export function LiveGameView({ slug, isOpen, onClose }: LiveGameViewProps) {
  ...
  useEffect(() => { if (isOpen && slug) loadGame(); }, [isOpen, slug]);
  ...
  const loadGame = async () => {
    ...
    try {
      ...
      if (url) { setGameUrl(url); } else { setError('Não foi possível gerar o link.'); }
    } catch (err) { setError('Erro ao gerar link'); } finally { setLoading(false); }
  };
    ...
    <div id="live-game-container" className="bg-bg-card border border-border-default rounded-2xl overflow-hidden">
      <div className="flex items-center justify-between p-3 bg-bg-secondary/80 border-b border-border-default">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold text-text-primary">{roleta?.nome || slug}</span>
        ...
        <div className="flex items-center gap-1">
          <button onClick={loadGame} disabled={loading} className="p-1.5 rounded-lg hover:bg-bg-tertiary text-text-muted hover:text-text-primary transition-colors disabled:opacity-50">
        ...
        {loading ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center max-w-md px-4">
              ...
              <button onClick={loadGame} className="px-6 py-2 rounded-xl text-sm font-medium text-white" style={{ backgroundColor: cor }}>
                <RefreshCw className="w-4 h-4 inline mr-2" /> Tentar novamente
              </button>
            </div>
          </div>
        ) : gameUrl ? (
          <iframe src={gameUrl} className="w-full h-full border-0" allow="autoplay; fullscreen; camera; microphone; accelerometer; gyroscope" loading="lazy" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-modals allow-orientation-lock" />
        ) : (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <div className="text-6xl mb-4">🎰</div>
              <p className="text-text-muted">Clique em "Gerar link" para começar</p>
              <button onClick={loadGame} className="mt-4 px-6 py-2 rounded-xl text-sm font-medium text-white" style={{ backgroundColor: cor }}>
                ▶ Gerar link do jogo
              </button>
            </div>
          </div>
      ...
      <div className="p-2 bg-bg-secondary/50 border-t border-border-default">
        <div className="flex items-center justify-between text-[10px] text-text-muted">
          ...
          <span className="flex items-center gap-1">
            {gameUrl ? <><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /><span>Conectado</span></> : <span>Aguardando</span>}
          </span>
        </div>
      </div>
    </div>
